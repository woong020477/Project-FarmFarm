...
![FarmFarm 로그인 화면 — Google 계정 로그인과 자동 로그인 선택](info_img/1.png)
...
![FarmFarm 플레이 화면 — 농지, 드론, 창고와 수송 헬기](info_img/2.png)

도시 속 농장에서 물주기·심기·수확을 제어하고, 헬기 도착 시간과 바리케이드 체력·경험치를 확인합니다.
...
![농지 선택과 작물 변경 — 작물별 이미지, 보유량, 판매가, 성장 시간과 수확량](info_img/9.png)

농지를 선택하면 심을 작물을 변경할 수 있으며, 잠긴 농지는 해금에 필요한 레벨을 표시합니다.
...
![농작물 판매 — 상추 판매 수량 입력과 수송 헬기 이륙까지 남은 시간](info_img/3.png)
...
![퀘스트 목록 — 제출 작물과 수량, 골드·경험치 보상](info_img/7.png)

퀘스트마다 필요한 작물과 보유량, 제출 보상을 표시하고 다음 생성 시간과 일일 기본 경험치 횟수를 안내합니다.

![기간 이벤트 — 병력 호출 비용 감소 효과와 남은 시간](info_img/6.png)

현재 이벤트의 안내 문구, 남은 시간, 적용 대상과 변화율을 한 화면에서 확인합니다.
...
![막사 — 보병, 탱크, 헬기의 고용 비용과 전투 능력치](info_img/4.png)

막사에서 현재 인구와 골드를 확인하고, 병력별 체력·공격력·공격 빈도를 비교합니다.

![기지 방호 — 바리케이드 체력과 골드를 사용한 수리](info_img/8.png)

방호 화면은 현재 체력과 붕괴 시 보호 규칙을 안내하며, 보유 골드로 수리 가능한 체력과 비용을 표시합니다.
...
![설정 화면 — BGM과 SFX 볼륨 슬라이더 및 크레딧](info_img/5.png)
